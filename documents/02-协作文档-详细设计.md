...
- 进入回收站后，详情、分享和 CRDT 鉴权均按不存在处理，并断开该文档已有协作连接。
- 恢复清除删除标记；彻底删除同时清理附件、权限、版本、批注以及 `crdt_update`、当前 checkpoint 和历史 checkpoint。
...
- 删除文件夹采用"内容上提"语义：把该文件夹内的文档与直接子文件夹移动到**最近的未删除祖先**（父级链上已被删除或不存在时继续上溯，最终回落到根），然后只把该文件夹自身软删除。该操作与内容移动在同一个事务内完成，因此不会产生指向已删除文件夹的悬空引用。前端删除前必须二次确认，并展示影响范围（N 个文档 / M 个子文件夹）。
...
6. 协作者权限变更与撤销会写入 `operation_log`，记录操作者、目标用户、旧/新权限、文档和请求 IP；变更后通过内部管理接口断开旧连接，重连时重新鉴权。
...
- DOCX/PDF HTTP 接口保持直接下载语义，生成任务由 Spring MVC 交给独立有界线程池执行，避免长时间占用普通请求线程；线程数、队列和超时可通过 `EXPORT_EXECUTOR_*`、`EXPORT_TIMEOUT_MS` 配置。
- 当前服务端生成器仍需在内存中形成最终文件，并通过 500 万字符请求上限约束单次输入。超大文档若超出现有容量基线，应升级为任务 ID、落盘或对象存储下载方案，不能宣称当前已经实现分片流式导出。
...
`doc_version` 保存用户可读的命名版本、说明、正文和来源修订。版本对比采用左右布局并突出插入、删除或变化区域。CRDT 版本回滚先在隔离 Y.Doc 上生成目标状态并持久化 checkpoint，成功后再把同源 Yjs 增量应用到在线文档，避免只修改物化 HTML 或在 checkpoint 失败时留下半完成状态。
